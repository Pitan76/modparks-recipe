...
/** ページ組み立てに要る値。 */
type ShellProps = { locale: string; origin: string; t: SearchMessages };
...
export function searchPage(locale: string, origin: string): string {
  ...
  return `<!DOCTYPE html>${(<Shell locale={locale} origin={origin} t={t} />).toString()}`;
}

/** ページ全体の器。 */
function Shell({ locale, origin, t }: ShellProps) {
  return (
    <html lang={locale}>
      <head>
        <meta charset="UTF-8" />
 ...
 */
function OgTags({ locale, origin, t }: ShellProps) {
