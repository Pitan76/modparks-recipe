...
export interface Env {
  ...
  // ローカル開発モード（`.dev.vars` にのみ書きます）。`true` のとき、プロバイダを通さない
  // ログインが生え、日次の投稿枠を数えなくなります。判定は utils/dev.ts に集めています。
  DEV_MODE?: string;
  // jar 解析 Worker への Service Binding。外部 jar のアップロードを開く場合のみ設定します。
