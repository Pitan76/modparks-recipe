 ...
 */
export async function renderRecipeGif(recipeData: any, env: Env, maxFrames: number = 5, scale: number = DEFAULT_SCALE, src: AssetReader = legacyAssetSource(env), tagNamespaces: TagNamespaceFilter = DEFAULT_TAG_NAMESPACES): Promise<Uint8Array> {
  await ensureWasm();
  const frames = [];
