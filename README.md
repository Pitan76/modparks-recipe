...
レシピ画像1枚の生成は、素材ごとのR2読み取りとラスタライズを伴うため素では重い処理です。次の3段構えで、同じ画像を2度作らないようにしています。

| 段 | 対象 | 効果 |
| --- | --- | --- |
| 1 | **エッジキャッシュ**（Cache API） | 同一URLの再リクエストは再生成せずキャッシュから返します。TTLは `Cache-Control: public, max-age=86400`（1日）。 |
| 1b | **アセットバージョン** | キャッシュキーに名前空間ごとのバージョンを含め、書き込みAPI実行時に更新します（下記）。 |
| 2 | **`render3d/` への保存** | 3Dブロックアイコンは初回生成時にR2へ保存され、以降は解決順の段階1で即ヒットします。 |
| 3 | **リクエスト内メモ化＋並列化** | 9スロット＋結果を並列に解決し、同一アイテムIDは1リクエストにつき1回しか読みません（同じ素材が複数スロットに並ぶレシピで特に効きます）。 |
| 4 | **モデル／テクスチャのメモ化** | 3D生成が読むモデルJSONとテクスチャを、isolate内でアセットバージョン単位にキャッシュします（[src/utils/block-icon.ts](src/utils/block-icon.ts)）。 |

段4がなぜ要るか: 1アイコンの生成中に同じオブジェクトを何度も読み直します。`parent` チェーンを `item/<path>` で1回、`block/<path>` でもう1回、`renderModelToSvg` の中でさらに1回辿るうえ、`block/cube_all` のような共通の親は全スロットが要求します。これらは直列なので素直に積み上がります。`wrangler dev --remote` での実測では、ウォーム状態の `minecraft:stone` 1個が**約4.6秒 → 約250ms**になりました。値ではなくPromiseをキャッシュしているため、並列スロットが同じ親を同時に要求しても実R2読み取りは1回に合流します。

### キャッシュ破棄（アセットバージョン）

書き込みAPI（レシピ・テクスチャ・モデル・タグのいずれか）を実行すると、その名前空間の**アセットバージョン**が更新されます。バージョンは `meta/version/<namespace>` にR2保存され、画像URLのキャッシュキーに `__v=<version>` として含まれます。バージョンが変われば以前のキャッシュエントリは参照されなくなり、次のリクエストで再生成されます。

個別の `cache.delete()` ではなくキーにバージョンを混ぜているのは、次の2点のためです。

- テクスチャ1枚の差し替えが**どのレシピ画像に影響するか特定できない**（そのテクスチャを使う全レシピが対象）
- 1つのレシピ画像に `?scale=` `?tagOffset=` のクエリ違いで**複数のキャッシュエントリ**が存在する

バージョンはWorkerのisolate内で10秒間メモ化されるため、通常のリクエストでR2読み取りは発生しません。**アップロードの反映は最大10秒遅れます。**
